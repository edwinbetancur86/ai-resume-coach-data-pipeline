...
| 24 | step3 is OUR ≤3-attempt loop (LLM returns lenient GenResume, WE gate vs strict + feed the new errors back each attempt); metadata/injected_defect STRIPPED before the corrector sees it; a list-shrink check guards against deletion cheats | Spec §9 mandates our explicit error-feedback loop; hiding ground truth keeps the metric honest; shrink-check catches "delete the bad entry" fixes that re-validation alone would pass. Result: 46/46 fixed in 1 attempt, valid 84.7%→100% | Metric #4, Rule #4 |
